...
object WordCountBetterSortedDataset {
  ...
  def main(args: Array[String]) {
    ...
    input.createOrReplaceTempView("book")
    ...
    val words = spark.sql("""
      WITH
        ...
        words_to_columns_filteres as (
          SELECT
            word
          FROM words_to_columns
          WHERE word <> ""
        )
        SELECT
          word,
          count(*) as ocurrences
        FROM words_to_columns_filteres
        GROUP BY word
        ORDER BY ocurrences ASC
    """)

    words.show(words.count.toInt, false)
    spark.stop()
